...
import { DRINKS, type Drink } from "../types";

interface DrinkOrderProps {
  onOrder: (drink: Drink) => void;
}

export default function DrinkOrder({ onOrder }: DrinkOrderProps) {
    ...
    <div className="px-6 pb-12 max-w-[600px] mx-auto bg-[#0f172a]">
      <div className="flex items-center justify-between mb-6 mt-2 px-2">
        <h2 className="text-sm font-medium text-slate-400 tracking-widest uppercase">
          {isOrdering ? "Processing..." : "Select Menu"}
        </h2>
          ...
          >
            <div className="flex items-center gap-5">
              ...
              <div className="text-left">
                <div
                  className={`text-lg font-medium tracking-wide ${drink.textColor}`}
                >
                  {drink.name}
                </div>
            ...
            <div
              className={`text-slate-600 transition-transform duration-300 ${
                isOrdering
                  ? ""
                  : "group-hover:translate-x-1 group-hover:text-slate-400"
              }`}
            >
              <svg
                className="w-5 h-5"
        ...
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-50 animate-toast">
          <div className="bg-[#1e293b] text-slate-200 pl-4 pr-6 py-3 rounded-md shadow-2xl border-l-4 border-[#cca43b] flex items-center gap-4">
            ...
            <div>
              ...
              <div className="font-medium text-sm">{selectedDrink} ordered</div>
            </div>
          </div>
        </div>
      )}
